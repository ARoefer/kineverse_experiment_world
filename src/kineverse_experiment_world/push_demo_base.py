...
def generate_push_closing(km, grounding_state, controlled_symbols, eef_pose, obj_pose, eef_path, obj_path, nav_method='cross', cp_offset=0):
    ...
    true_geom_distance = gm.dot_product(contact_normal, robot_cp - object_cp)
    coll_world    = km.get_active_geometry(gm.free_symbols(true_geom_distance))
    ...
    if nav_method == 'linear':
        geom_distance = gm.norm(object_cp + active_tangent * true_geom_distance - robot_cp)
    elif nav_method == 'cubic':
        dist_scaling  = 2 ** (-0.5*((true_geom_distance - 0.2) / (0.2 * 0.2))**2)
        ...
        geom_distance = true_geom_distance
    ...
    def debug_draw(vis, state, cmd):
        ...
        vis.draw_vector('debug_vecs', s_object_cp, gm.subs(active_tangent * true_geom_distance, state), r=0, b=1)
