...
if __name__ == '__main__':
	...
	controllers = {j: JointController(j, f'/{j[:-6]}_velocity_controller/command') for j in joints}
	...
	def process_js_command(cmd):
		if len(cmd.name) != len(cmd.velocity):
			print(f'Received invalid command. |name| = {len(cmd.name)} |velocity| = {len(cmd.velocity)}')
			return

		for name, vel in zip(cmd.name, cmd.velocity):
			if name in controllers:
				controllers[name].send_command(vel)
			else:
