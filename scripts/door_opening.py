...
class ConditionalDoorHandleConstraints(Operation):
    ...
    def _execute_impl(self, door_position, handle_position, locking_tolerance, handle_release_angle):
        ...
        is_locking = gm.alg_and(gm.less_than(door_position, locking_tolerance), gm.less_than(handle_position, handle_release_angle))
        self.constraints = {f'lock {door_position}': Constraint(-1e9, is_locking * 1e9, door_vel)}

def create_door(km, prefix, height, width, frame_width=0.05):
    ...
    frame_pillar_l_geom = Box(prefix + ('frame',), gm.translation3(0, 0.5 * (width + frame_width), 0.5 * height),
                                                   gm.vector3(frame_width, frame_width, height))
    frame_pillar_r_geom = Box(prefix + ('frame',), gm.translation3(0, -0.5 * (width + frame_width), 0.5 * height),
                                                   gm.vector3(frame_width, frame_width, height)) 
    frame_bar_geom      = Box(prefix + ('frame',), gm.translation3(0, 0, height + 0.5 * frame_width),
                                                   gm.vector3(frame_width, width, frame_width)) 
    frame_rb   = RigidBody(Path('world'), gm.eye(4), geometry={1: base_plate_geom, 
...
if __name__ == '__main__':
    ...
    with open(res_pkg_path('package://kuka_lbr_iiwa_support/urdf/lbr_iiwa_14_r820.urdf'), 'r') as f:
        load_urdf(km, Path('iiwa'), urdf_filler(URDF.from_xml_string(hacky_urdf_parser_fix(f.read()))))

    km.clean_structure()
    km.dispatch_events()

    create_door(km, Path('door'), 0.5, 0.35)

    km.clean_structure()
    km.dispatch_events()

    door = km.get_data('door')
    symbols = gm.free_symbols(door.links['handle'].pose)
