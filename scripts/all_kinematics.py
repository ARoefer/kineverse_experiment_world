...
def simple_kinematics(km, vis):
    ...
    # constraint      = Constraint(-limi_ang, limi_ang, gm.asin(gm.norm(cross(rotation_vector, neutral_axis))))

    # # Planar
    # c = gm.Position('c')
    # parent_frame    = gm.frame3_axis_angle(gm.vector3(0,0,1), 0.56, gm.point3(0,-5,0))
    # child_frame     = parent_frame * gm.translation3(a, b, 0) * gm.rotation3_axis_angle(gm.vector3(0, 0, 1), c)
    ...
    # gm.frame3_axis_angle(gm.vector3(1,0,0), gm.asin((-sin(SYM_ROTATION) * 0.05 / 0.15)), rotator_frame * gm.point3(0, 0, 0.05))
    piston_to_rod   = gm.frame3_axis_angle(gm.vector3(1,0,0), gm.asin(gm.sin(SYM_ROTATION) * 0.05 / 0.15), gm.point3(0,0,0.15))
    piston_frame    = gm.dot(rod_frame, piston_to_rod) # gm.translation3(*(rod_frame * gm.point3(0,0,0.15)))
    ...
    ph_l_obj = RigidBody('pb_l', gm.dot(parent_frame, gm.dot(pb_l_to_parent, ph_l_to_pb)), 
    ...
    pb_r_obj = RigidBody('mdj_base', gm.dot(parent_frame, pb_r_to_parent), 
                                  ...
                                  geometry={ 1: thick_box},
                                  collision={1: thick_box})
    ph_r_obj = RigidBody('pb_r', gm.dot(parent_frame, gm.dot(pb_r_to_parent, ph_r_to_pb)), 
                                  to_parent=ph_r_to_pb,
                                  geometry={ 1: thin_box},
                                  collision={1: thin_box})
    bar_obj  = RigidBody('mdj_base', gm.dot(parent_frame, gm.dot(bar_pitch, bar_roll)),
...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        ...
        if now - last_update >= (1.0 / 50.0):
            state = {s: gm.sin(time() + x * 0.1) for x, s in enumerate(symbols)}
    ...
    # parent_frame = gm.frame3_axis_angle(gm.vector3(0,0,1), 0.56, gm.point3(0,0,0))
    # child_frame  = parent_frame * gm.translation3(a, 0, 0)


    # # HINGE
    # parent_frame = gm.frame3_axis_angle(gm.vector3(0,0,1), 0.56, gm.point3(0,-1,0))
    # child_frame  = parent_frame * gm.rotation3_axis_angle(gm.vector3(0,1,0), a)

    # # CYLINDRICAL 
    # b = gm.Position('b')
    # parent_frame = gm.frame3_axis_angle(gm.vector3(0,0,1), 0.56, gm.point3(0,-2,0))
    # child_frame  = parent_frame * gm.translation3(a, 0, 0) * gm.rotation3_axis_angle(gm.vector3(1,0,0), b)

    # # SCREW
    # thread_pitch = 2  # Millimeters per revolution
    # parent_frame = gm.frame3_axis_angle(gm.vector3(0,0,1), 0.56, gm.point3(0,-3,0))
    # child_frame  = parent_frame * gm.translation3((thread_pitch * 1e-3) * (a / pi), 0, 0) * gm.rotation3_axis_angle(gm.vector3(1,0,0), a)

    # # BALL
    # ax, ay, az      = [gm.Position('a{}'.format(x)) for x in 'xyz']
    # rotation_vector = gm.vector3(ax, ay, az)
    # neutral_axis    = gm.vector3(1, 0, 0)
    # parent_frame    = gm.frame3_axis_angle(gm.vector3(0,0,1), 0.56, gm.point3(0,-4,0))
    # child_frame     = parent_frame * gm.rotation3_axis_angle(rotation_vector / (gm.norm(rotation_vector) + 1e-5), gm.norm(rotation_vector))
    ...
    # # Remote Control
    # rc_pos          = GC(a, {b: 1})
    # parent_frame    = gm.frame3_axis_angle(gm.vector3(0,0,1), 0.56, gm.point3(0,-8,0))
    # child_frame     = parent_frame * gm.translation3(rc_pos)

    # # Mimic - Trashcan
    # parent_frame    = gm.frame3_axis_angle(gm.vector3(0,0,1), 0.56, gm.point3(0,-9,0))
    # child1_frame    = parent_frame * gm.rotation3_axis_angle(gm.vector3(0, 1, 0),      a)
    # child2_frame    = parent_frame * gm.rotation3_axis_angle(gm.vector3(0, 1, 0), 5 * -a)

    # # Roomba
    # lx, ly, la = [gm.Position('l{}') for x in 'xya']
    # jx, jy, ja = [Velocity('j{}') for x in 'xya']
    # bx = GC(lx, {jx: cos(la), jy: sin(la)})
    # by = GC(ly, {jx: sin(la), jy: cos(la)})
    # ba = GC(la, {ja: 1})
    # parent_frame = gm.frame3_axis_angle(gm.vector3(0,0,1), 0.56, gm.point3(0,-10,0))
    # child_frame  = parent_frame * gm.translation3(bx, by, 0) * gm.rotation3_axis_angle(gm.vector3(0, 0, 1), ba)

    # # COMPARTMENT
    # parent_frame       = gm.frame3_axis_angle(gm.vector3(0,0,1), 0.56, gm.point3(0,-10,0))
    # door_frame         = parent_frame * gm.translation3(0.2, 0.2, 0) 
    #                                   * gm.rotation3_axis_angle(gm.vector3(0, 0, 1), a) 
    #                                   * gm.translation3(0, -0.2, 0)
    # handle_frame       = door_frame * gm.translation3(0, 0.02, 0) 
    #                                 * gm.rotation3_axis_angle(gm.vector3(1, 0, 0), b)
    # unlock_position    = 1.0
    # locking_constraint = Constraint(-1e9, greater_than(b, unlock_position) * 1e9, DiffSymbol(a))

    # # Microwave
    # parent_frame       = gm.frame3_axis_angle(gm.vector3(0,0,1), 0.56, gm.point3(0,-11,0))
    # door_frame         = parent_frame * gm.translation3(0.2, 0.2, 0) 
    #                                   * gm.rotation3_axis_angle(gm.vector3(0, 0, 1), a) 
    #                                   * gm.translation3(0, -0.2, 0)
    # button_frame       = parent_frame * gm.translation3(0, 0.02, 0) * gm.rotation3_axis_angle(gm.vector3(1, 0, 0), b)
    # unlock_position    = 1.0
    # locking_constraint = Constraint(-1e9, greater_than(b, unlock_position) * 1e9, DiffSymbol(a))
